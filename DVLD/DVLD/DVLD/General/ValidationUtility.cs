...
namespace DVLD
{
    internal class ValidationUtility
    {
        ...
        public static bool IsValidNameCharacter(char character, out string errorMessage)
        {
            ...
            errorMessage = "";
            if (!char.IsWhiteSpace(character)&& !char.IsLetter (character) && character != (char)Keys.Delete && character != (char)Keys.Back)
            {
        ...
        public static bool IsValidUserNameCharacter(char character, out string errorMessage)
        {

            errorMessage = "";
            if (!char.IsLetterOrDigit(character)&& character != (char)Keys.Delete && character != (char)Keys.Back)
            {
                errorMessage = "Please enter only a letter";
